...
class FavoriteService {
    async getAll(refreshToken: string) {
        try {
            ...
            const favoriteOwner = await pool.query<IFavorite>('SELECT * FROM favorite where user_id = $1', [user.id])
    ...
    async create(refreshToken: string, productId:number) {
        ...
        try {
            const user = await authService.check(refreshToken)
            const createdFavoriteOwner = await pool.query('INSERT INTO favorite (user_id, product_id) values ($1,$2) RETURNING *', [user.id, productId])
    ...
    async delete(refreshToken: string, productId:number){
        try {
            const user = await authService.check(refreshToken)
            const deletedFavorite = await pool.query('DELETE FROM favorite where user_id = $1 and product_id = $2 RETURNING *', [user.id, productId])
